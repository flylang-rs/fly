...
macro_rules! common_operation_binary {
    ($name:ident, $ty1:ident, $ty2:ident, $res_ty:ident, $conv:expr) => {
        pub fn $name(_realm: $crate::SharedRealm, args: &[$crate::object::Value]) -> $crate::control_flow::ControlFlow {
            let lhs = &args[0];
            let rhs = &args[1];

            if let $crate::object::Value::$ty1(x) = lhs && let $crate::object::Value::$ty2(y) = rhs {
                return $crate::control_flow::ControlFlow::Return($crate::object::Value::$res_ty(($conv)(x, y)));
            }
    ...
    ($name:ident, $ty:ident, $res_ty:ident, $conv:expr) => {
        pub fn $name(_realm: $crate::SharedRealm, args: &[$crate::object::Value]) -> $crate::control_flow::ControlFlow {
            ...
            if let $crate::object::Value::$ty(x) = val {
                return $crate::control_flow::ControlFlow::Return($crate::object::Value::$res_ty(($conv)(x)));
            }
